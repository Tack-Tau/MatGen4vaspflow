...
class BooleanEmbeddingVector(BaseUnconditionalEmbeddingModule):
    # If True, we don't need conditional values to evaluate an unconditional score
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: torch.Tensor of shape (batch_size,) or (batch_size, 1) containing 0s and 1s
        """
        # if x.dim() == 2:
        #     x = x.squeeze(-1)  # (batch_size, 1) -> (batch_size,)
